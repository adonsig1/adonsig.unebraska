	...
	<body>
		...
		<?php
			...
			$query =
				...
				from
				  teaching t inner join instructors i on t.instructor = i.id
				    inner join role_types r on i.role = r.id
				where
				  session = '$session'
				group by i.role";
		...
		<?php 
			...
			$query =
				...
				FROM 
				  classes c INNER JOIN courses co ON c.course = co.id 
				WHERE 
				  c.session='$session' AND (ISNULL(c.instructor) OR c.instructor = '' OR c.instructor = 'OPEN')";
			$rs = mysqli_query($newconn, $query);
			$row = mysqli_fetch_row($rs);
