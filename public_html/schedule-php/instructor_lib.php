<?php
	...
	function get_class_object($classId, $session) {
		...
		$rs = mysqli_query($newconn, "SELECT * FROM classes WHERE id = '$classId' AND session = '$session'");
		return mysqli_fetch_object($rs);
	}
	...
	function get_prefs_comments($id, $session) {
		...
		$rs = mysqli_query($newconn, "SELECT comment FROM pref_comments WHERE instructor = '$id' AND session='$session'");
	...
	// assumes instructor is not a filler
