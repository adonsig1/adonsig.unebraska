<?php
	...
	$session = get_academic_session();
        ...
        if ( isset($_REQUEST["instrId"]) ) {
	        $instrId = mysqli_real_escape_string($newconn, $_REQUEST["instrId"]);
        } else {
                $instrId = "";
        }
	...
	if ($action == 'list_instructors_ajax') {
		...
		do_delete_class(mysqli_real_escape_string($newconn, $_REQUEST["classId"]));
	} else if ($action == 'assinn_class') {
		do_assign_class(mysqli_real_escape_string($newconn, $_REQUEST["classId"]));
	}
	...
	<body onLoad="onLoad()">
		<?php
			...
			include 'std_header.php'; 
	        		...
	        		$classId = mysqli_real_escape_string($newconn, $_REQUEST["classId"]);
        		} else {
                		$classId = "";
        		}
