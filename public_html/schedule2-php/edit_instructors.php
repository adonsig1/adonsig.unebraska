		...
		<script type="text/javascript">
			var data = {
				<?php
					...
					while ($x = mysql_fetch_object($rs, 'Instructor')) {
						output_json_row($x);
					}
				?>
			};
		...
		<form name="dataForm">
			<table class="list">
				<?php
					...
					mysql_data_seek($rs, 0);
					while ($instr = mysql_fetch_object($rs, 'Instructor')) {
						output_row($instr);
					}
				?>
			</table>
...
// TODO: fix HACK - remove 'php' on line above.	
	...
	function do_add() {
		...
		$ln = mysql_real_escape_string($_REQUEST['ln']);
		$fn = mysql_real_escape_string($_REQUEST['fn']);
		$email = mysql_real_escape_string($_REQUEST['email']);
		$started = mysql_real_escape_string($_REQUEST['started']);
		...
		mysql_query($query);
		// update time-depedent instructor_roles TBR
		$query = "INSERT INTO instructor_roles (id, ln, fn, role, started, session, Session_order)
			SELECT '$id', '$ln', '$fn', '$role', $started', s.id, s.order
			FROM sessions s
			WHERE s.name = '$started'";
		mysql_query($query);
	}
	...
	function do_delete() {
		...
		mysql_query($query);
		// Delete from time-dependent instructor_roles TBR
		$query = "DELETE FROM instructor_roles WHERE id = '$id'";
		mysql_query($query);
	}
