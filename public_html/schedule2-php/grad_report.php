<?php 
	...
	function get_grads_hours_list($session) {
		$query = 
			"SELECT 
			...
			FROM 
			  ...
			  # Next line implements time-dependent roles TBR #
			  INNER JOIN instructor_roles ir ON ir.id = i.id AND ir.Session_order = (SELECT MAX(ir2.Session_order) FROM instructor_roles ir2 JOIN sessions s2 WHERE s2.id = '$session' AND ir2.Session_order <= s2.order AND ir2.id = ir.id)
			WHERE t.session = '$session' AND (ir.role = 'gta' OR ir.role = 'pd' OR ir.role = 'lec')
			GROUP BY i.id
			ORDER BY ir.role,i.ln, i.fn";
...
<html>
	...
	<body>
		...
		<table class="display">
			<?php
				$list = get_grads_hours_list($session);
				foreach ($list as $x) {
					if ($x->surplus > 0) {
					...
					echo ($x->role == 'lec' || $x->role == 'pd') ? '<td style="font-weight: bold;">' : '<td>';
