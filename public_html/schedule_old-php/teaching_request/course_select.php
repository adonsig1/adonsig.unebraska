<?php 
	...
	$rs_sel = mysql_query($query_sel);
	$rs_non = mysql_query($query_non);
	
	while ($r = mysql_fetch_assoc($rs_sel)) {
		$k = strlen($r["text"]);
		$max_width = ($k > $max_width) ? $k : $max_width; 
	}
	mysql_data_seek($rs_sel, 0);
	
	while ($r = mysql_fetch_assoc($rs_non)) {
		$k = strlen($r["text"]);
		$max_width = ($k > $max_width) ? $k : $max_width; 
	}
	mysql_data_seek($rs_non, 0);
?>
	...
	<tr>
		...
		<td>
			...
			<div  class="small_instructions" style="width:<?php echo $max_width ?>ex;">
				This is an ordered list, with highest priority
